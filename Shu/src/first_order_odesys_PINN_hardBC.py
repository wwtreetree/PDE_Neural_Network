...
class ORDER1_ODESYS_PINN_HARDBC(nn.Module): 
    ...
    def __init__(self, f, lb, ub, BC, 
        ...
        # f       :  y' = f(x,y) tensor function acted on tensors x and y of size [ batch_size , 1 ]
        ...
        self.model_type = 'ORDER1_ODESYS_PINN_HARDBC'
        self.f = f
    ...
    def ResidualLoss(self, x, y_hat):
        ...
        f_hat   = self.f(x, y_hat)
        Lp = ((D1y_hat - f_hat)**2).sum()/len(x) # compute average loss for each sample
        return Lp 
